...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // Extract ticketId from path: /.netlify/functions/ticket-echanges-[ticketId]
    const pathParts = event.path.split('/');
    const functionName = pathParts[pathParts.length - 1]; // ticket-echanges-[ticketId]
    const ticketId = functionName.replace('ticket-echanges-', '');

    if (!ticketId || ticketId === 'ticket-echanges') {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ detail: 'ID du ticket manquant dans l\'URL' })
      };
    }
