...
const ProductionTacheModal = ({ tache, onClose, onRefresh }) => {
  ...
  const [showTrunkTemplate, setShowTrunkTemplate] = useState(false);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-4xl w-full max-h-[95vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-hidden flex">
          ...
          <div className="flex-1 flex flex-col">
            ...
            {editingTache && user?.type_utilisateur === 'agent' ? (
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {loadingCommentaires ? (
                  ...
                  (Array.isArray(commentaires) ? commentaires : []).map(commentaire => {
                    const isAgent = commentaire.auteur_type_real === 'agent';
                    const isCurrentUser = commentaire.auteur_id === user?.id;
                          ...
                          <div className={`bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg p-3 ${
                            ...
                            <div className="flex items-center justify-between mb-2">
                              <div className="font-medium text-sm text-gray-900 dark:text-dark-text">
                                ...
                                {isAgent && <span className="ml-1 text-xs text-blue-600 dark:text-blue-400">(Agent)</span>}
                                {!isAgent && <span className="ml-1 text-xs text-green-600 dark:text-green-400">(Demandeur)</span>}
                              </div>
